...
class SmartBedDataset_Train(SmartBedDataset_Base):
    ...
    def __init__(self, ori_dataset, model):
        ...
        train_ratio = 0.7
        if not os.path.exists("dataset/all_files_for_train/idx_train") and not os.path.exists(
                "dataset/all_files_for_train/idx_vali"):
            ...
            torch.save(train_idxs, "dataset/all_files_for_train/idx_train")
            torch.save(vali_idxs, "dataset/all_files_for_train/idx_vali")
        else:
            train_idxs = torch.load("dataset/all_files_for_train/idx_train")
            vali_idxs = torch.load("dataset/all_files_for_train/idx_vali")
...
if __name__ == '__main__':
    ...
    print(f"target_mean: {train_dataset.target_mean.shape}")  # target_mean: torch.Size([1, 32, 64])
    print(f"target_std: {train_dataset.target_std.shape}")  # target_std: torch.Size([1, 32, 64])
    print(f"input_mean: {train_dataset.input_mean.shape}")  # input_mean: torch.Size([12, 32, 64])
    print(f"input_std: {train_dataset.input_std.shape}")  # input_std: torch.Size([12, 32, 64])

    print(f"target_mean: {train_dataset.target_mean}")  # target_mean: torch.Size([1, 32, 64])
    print(f"target_std: {train_dataset.target_std}")  # target_std: torch.Size([1, 32, 64])
    print(f"input_mean: {train_dataset.input_mean}")  # input_mean: torch.Size([12, 32, 64])
    print(f"input_std: {train_dataset.input_std}")  # input_std: torch.Size([12, 32, 64])
