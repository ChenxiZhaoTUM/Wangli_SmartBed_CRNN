...
for epoch in range(epochs):
    ...
    for i, traindata in enumerate(trainLoader, 0):
        ...
        if epoch % 500 == 0 and i in random_indices:
            ...
            #     utils.makeDirs(["TRAIN_CRNN_expo4_50files"])
            #     utils.imageOut("TRAIN_CRNN_expo4_50files/epoch{}_{}_{}".format(epoch, i, j), inputs_cpu[j],
    ...
    with torch.no_grad():
        for i, validata in enumerate(valiLoader, 0):
            ...
            # if epoch % 500 == 0 and i in random_indices:
            #     for j in range(batch_size):
            #         utils.makeDirs(["VALIDATION_CRNN_expo4_50files"])
            #         utils.imageOut("VALIDATION_CRNN_expo4_50files/epoch{}_{}_{}".format(epoch, i, j), inputs_cpu[j],
            #                        targets_denormalized[j], outputs_denormalized[j])
